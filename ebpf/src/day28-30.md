...
## Option A — reference solution

The five options above are *yours* to build; the point of the capstone is to
design one without a guide. But so this book compiles and ships one complete,
end-to-end example, the lab includes a full reference implementation of
**Option A** — a generic latency tracer — in `ebpf/labs/day28-30/`. Read it
*after* you've attempted your own; treat it as one answer, not the answer. The
other four options remain deliberately unimplemented exercises.

It is the standalone libbpf pattern from Days 1–13: a shared header, a BPF
object, and a userspace loader, built by `make -C ebpf/labs day28-30`.

The shared layout — the per-stack histogram and the outlier record both sides agree on:

{{#include ../labs/day28-30/capstone_latency.h}}

The hand-built branchless `log2l` (Prerequisite 1 — there is no `lhist` helper):

{{#include ../labs/day28-30/capstone_latency.bpf.c:log2}}

The full BPF object: fentry stamps an entry time, fexit computes the duration,
buckets it into the per-stack `PERCPU_HASH` histogram (one increment, no atomics),
and emits a ring-buffer record with kernel + user stack ids only for outliers
above the threshold:

{{#include ../labs/day28-30/capstone_latency.bpf.c:book}}

The userspace loader points both programs at a function chosen with `-f`
(default `vfs_read`) via `bpf_program__set_attach_target`, drains the outlier
ring buffer while symbolizing kernel frames from `/proc/kallsyms`, and reads the
whole histogram once on exit — summing the per-CPU slots as the check question
below demands:
