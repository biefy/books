...
Included from the source the lab build and CI compile. (`TCX_NEXT` is a BTF
enum from `vmlinux.h`; the terminal `TC_ACT_*` codes are defined locally
because `<linux/pkt_cls.h>` cannot be mixed with `vmlinux.h`.)
...
The loader is built as `./tcx`; the listing is included from the source
the lab build and CI compile:
