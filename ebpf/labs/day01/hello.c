...
{
    const struct hello_event *event = data;
...
int main(void)
{
    ...
    if (!skel) {
        fprintf(stderr, "failed to open hello BPF skeleton\n");
        return 1;
    }

    err = hello_bpf__load(skel);
    if (err) {
        fprintf(stderr, "failed to load hello BPF programs: %s\n",
                strerror(-err));
        goto cleanup;
    }
    ...
    if (err) {
        fprintf(stderr, "failed to attach hello BPF programs: %s\n",
                strerror(-err));
        goto cleanup;
    }
    ...
    puts("Tracing unlink calls; press Ctrl-C to stop.");
    while (!exiting) {
