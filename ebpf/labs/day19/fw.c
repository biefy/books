...
int main(int argc, char **argv)
{
    ...
    skel = fw_bpf__open_and_load();
    if (!skel) {
        fprintf(stderr, "failed to open/load fw BPF skeleton\n");
        err = 1;
        goto cleanup;
    }

    link = bpf_program__attach_cgroup(skel->progs.block_udp, cg_fd);
    if (!link) {
        err = -errno;
        fprintf(stderr, "failed to attach cgroup_skb/egress to '%s': %s\n",
                cg_path, strerror(errno));
        goto cleanup;
    }

    printf("attached cgroup_skb/egress UDP filter to %s; Ctrl-C to detach\n",
           cg_path);
    while (!exiting)
        pause();
