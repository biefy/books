...
- **`sk_write_queue`** holds queued-but-not-yet-transmitted skbs; after transmission, TCP retains the originals in **`tcp_rtx_queue`** until ACK/free. **`sk_wmem_queued`** spans both queues and counts their charged memory (`skb->truesize`) against the **`sk_sndbuf`** cap. Full buffer ⇒ block (`sk_stream_wait_memory`) or `EAGAIN`. Auto-tuned by `tcp_wmem`.
...
**Answer:** In several send-side stages, not necessarily all in one queue. Bytes not yet transmitted remain in `sk_write_queue`; originals already transmitted but not ACKed sit in `tcp_rtx_queue`; transient transmit clones may be below TCP in IP/qdisc/device queues or on the wire. Because the BDP is small, most of the 1 MB will usually still be unsent when `send()` returns, but the exact split can change concurrently. The call returned because `sk_sndbuf` was big enough to accept the charged memory — not because the bytes left the box. If you read `/proc/<pid>/status` you'd see that memory in kernel TCP accounting, not in the process RSS. To inspect the split, run `ss -tim`: `unacked` counts sent-but-unacknowledged segments, while `skmem` `w` reports `sk_wmem_queued` across TCP's retained send-side skbs. `Send-Q` is the related-but-distinct `write_seq - snd_una` payload-byte count.
